...
### 阶段 D.3：设置页视觉收敛 —— 待执行（2026-09-08）

目标：让设置页继承当前桌面浅色工作台，而不是保留一套高对比的旧式工具面板。

- [ ] 移除工具诊断区域的深色卡片，统一使用 `Theme.surface`、`Theme.text` 和 `Theme.muted`，保证标题、版本、路径和提示均可读。
- [ ] 将 `yt-dlp`、`ffmpeg`、`ffprobe` 收敛为紧凑状态行；默认只展示名称、就绪状态和当前路径，版本、自定义路径和操作按需展开。
- [ ] 去除重复的“工具已就绪”提示：全局状态、侧栏状态和单工具状态各自只保留必要信息。
- [ ] 明确下载目录反馈：目录不存在时说明会创建，选择目录后提供明确的保存/应用动作和成功状态。
- [ ] 设置页缩小或隐藏空闲的右侧传输队列，避免其在配置场景占用主内容空间。

验收标准：

- macOS 宽屏下设置页与新建下载页保持同一浅色层级、圆角和按钮语言。
- 长版本号和路径不会挤压核心状态；用户可查看或复制完整值。
- 无下载任务时，设置页的主配置区域优先获得可用宽度。
